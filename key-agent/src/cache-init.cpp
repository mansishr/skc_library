...
__open_cache_connection(GError **error)
{
	...
	keyagent::localcache::connection_pointer = (gpointer)gda_connection_open_from_string (keyagent::localcache::provider_name->str,
		keyagent::localcache::connection_string->str, NULL, GDA_CONNECTION_OPTIONS_NONE, error);
...
__keyagent_cache_init(GError **err)
{
	...
	if(g_mkdir_with_parents(keyagent::localcache::database_directory->str, 0700)) {
		g_set_error(err, G_FILE_ERROR,	g_file_error_from_errno (errno), "Error making directory '%s': %s",
                     keyagent::localcache::database_directory->str, g_strerror (errno));
