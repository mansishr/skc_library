...
__keyagent_cache_loadkey_policy_attr(gint key_id, const char *attr_name, GError **error)
{
	...
	stmt = gda_sql_parser_parse_string(parser, "SELECT keyattr_policy_id, attr_value FROM key_policy_attributes WHERE key_id=##keyid::gint and attr_name=##attrname::gchararray", NULL, error);
...
__cache_key_policy_attr(keyagent_key_real *key, const char *attr_name, k_policy_buffer_ptr attr_value, gint key_id, GError **error)
{
	...
	if(!gda_connection_insert_row_into_table(GPOINTER_TO_GDA_CONNECTION(keyagent::localcache::connection_pointer),
        "key_policy_attributes", error, "key_id", &v_keyid, "attr_name", v_attrname, "attr_value", v_attrvalue, NULL)) {
		k_info_error(*error);
		 return FALSE;
	}
	gda_connection_commit_transaction(GPOINTER_TO_GDA_CONNECTION(keyagent::localcache::connection_pointer), NULL, NULL);
