...
//#define MAIN_ 1
#define IAS_BASE_URL "https://api.trustedservices.intel.com/sgx/dev"
#define IAS_VERSION "v3"
...
int main ( int argc, char **argv )
{
        ...
        if( argc < 3 )
	{
        ...
        stmsgx_epid_ssl_data::ias_base_url      = IAS_BASE_URL;
        stmsgx_epid_ssl_data::ias_version  	= IAS_VERSION;

        stmsgx_epid_ssl_data::ias_cacert  	= CA_CERT_PATH;
        stmsgx_epid_ssl_data::ias_sub_key      	= argv[2];

        quote_buffer_file                       = argv[3];
        if(quote_buffer_file==NULL)return 0;
#ifdef MAIN_
	stmsgx_epid_ssl_data::verify     	= TRUE;
	
      	if((access( stmsgx_epid_ssl_data::ias_cacert, F_OK ) == -1) || (access( quote_buffer_file, F_OK ) == -1))
	{
