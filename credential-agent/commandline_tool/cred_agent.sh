...
gen_csr_req()
{
	CSR_CMD="openssl req -newkey $KEY_TYPE:$KEY_BITS -nodes -keyout $PRIVATE_KEY_FILE -out $CSR_PEM_FILE -outform pem -subj \"$CERT_SUB\" -engine \"pkcs11\""
	check_last_cmd_exec_status "$CSR_CMD" "$RULE_ABORT" "CSR Request"
}

sign_with_cred_service()
{
	SIGN_CMD="curl -X POST ${CRED_URL} --data-binary @$CSR_PEM_FILE -H 'Content-Type: application/x-pem-file' -H 'Accept: application/x-pem-file' -H \"Authorization: Bearer $TOKEN_VALUE\" -k > $CERT_PEM_FILE"
