...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
	log_msg $LOG_WARN "Invalid argurement: $0 <CS_IP> <TOKEN> <CA_FILE>"
	exit -1
fi
cs_ip="$1"
token="$2"
...
#INVALID TOKEN_VALUE
value=$token"dfsdfs"
update_agent_config "$cs_ip" "${value}" "$ca" "$agent_conf_file"
execute_test "$(get_issue_cert_command \"\" force)" $CODE_EXEC_ERROR "1: Issue Certificate"

update_agent_config "$cs_ip" "$token" "$ca" "$agent_conf_file"
execute_test "$(get_issue_cert_command verbose force)" $CODE_EXEC_SUCCESS "2: Issue Certificate"

#AGENT_RENEW_CERT
execute_test "$(get_renew_cert_command verbose force config $agent_conf_file)" $CODE_EXEC_SUCCESS "3: Renew Certificate"
execute_test "$(get_renew_cert_command verbose force config $agent_conf_file)" $CODE_EXEC_SUCCESS "3: Renew Certificate"
...
execute_test "$(get_renew_cert_command)" $CODE_EXEC_SUCCESS "7: Renew Certificate without force"
