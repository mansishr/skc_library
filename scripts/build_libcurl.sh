...
exec_linux_cmd "git checkout ${DHSM2_COMPONENT_EXT_LIBCURL_VERSION}" $EXEC_RULE_ABORT "LIBCURL:checking out tag" $CODE_EXEC_ERROR
...
if [ -d ${DHSM2_COMPONENT_EXT_LIBCURL_INSTALL_DIR} ]; then
	rm -rf $DHSM2_COMPONENT_EXT_LIBCURL_INSTALL_DIR
fi
cmd="./configure --prefix=${DHSM2_COMPONENT_EXT_LIBCURL_INSTALL_DIR} --enable-http"
